...
const RegisterClientOptionDetails = () => {
  ...
  const [location, messages, getMobileNumber, getFormData] = useSelector((state) => [
    state.locationID,
    state.messages,
    state.mobileOTP,
    state.coreData,
...
// ================[Block START]================ //
  ...
  return (
    <Form onSubmit={(e) => getNext(e)} className='container px-0 my-4 d-flex flex-column gap-4' dir="rtl">
      <Row>
          {/* Full Name */}
          <Form.Group as={Col} sm={12} controlId="formGridFullName" className='px-0'>
              ...
              <Form.Control value={formInput.fullname} name="fullname" onChange={(e) => handleChange(e)}
                className={`${formMessages?.fullname ? 'uLT-bd-f-danger-sA' : 'uLT-bd-f-platinum-sA'} uLT-f-radius-sB cLT-main-text fLT-Regular-sB`} type='text' placeholder="الاسم بالكامل" />
      ...
      <Row className=" d-flex align-items-center mb-3 gap-3 gap-md-0">
        {/* Gender */}
        <Form.Group as={Col} md={6} controlId="formGridGender" className='mb-3 mb-md-0' >
          {/* Gender [Label] */}
          ...
          <div className={`uLT-bd-f-platinum-sA uLT-f-radius-sB cLT-main-text fLT-Regular-sB `}>
            <Select placeholder="النوع" options={getFormData?.gender} className='p-3~'
        ...
        {/* Nationality */}
        <Form.Group as={Col} md={6} controlId="formGridNationality" className='' >
          {/* Nationality [Label] */}
      ...
      <Row className='d-flex align-items-center'>
        ...
        <Form.Group as={Col} md={6} className="'mb-3  " >
          {/* Country [Option]  */}
          <div className={` uLT-bd-f-platinum-sA uLT-f-radius-sB cLT-main-text fLT-Regular-sB LT-edit-account-input mb-3 mb-md-0`} >
     ...
     <Row>
          ...
          <Form.Group as={Col} md={6} className="">
            ...
            {!location?.areaID && formMessages?.area_id && <p className='mb-0 fLT-Regular-sA cLT-danger-text pt-2 px-2'>{formMessages?.state_id}</p>}
          </Form.Group>
        </Row>
      ...
      <div className="d-flex flex-column  align-items-center justify-content-between gap-4">
           ...
           <label className='d-flex justify-content-center align-items-center gap-2 gap-sm-3 ' >
            <input id="acceptTerms"  checked={check}  type="checkbox" className='uLT-click' name="radio-button" onChange={() => setCheck(!check)} style={{ width: '24px', height: '24px' }} />
            <p className='mb-0  LT-agree-condition cLT-support2-text termsText'> اوافق علي الشروط والاحكام </p>
          </label>
        <div className="w-50">
          <ButtonShare loading={formLoading} type={!check} innerText={'إنشـــاء حســـاب'} btnClasses={'three cLT-secondary-bg'} textClasses={'py-1 px-4 cLT-white-text fLT-Regular-sB'} />
        </div>
        <div className="w-50">
