...
const RegisterFreelancerPersonalDetails = () => {
  ...
  const [formInput, setFormInput] = useState({ username: '', fullname: '', email: '', password: '', nationalID: '' })
  ...
  const validation = categorySkills?.length !== 0 && jobName?.id !== undefined && formInput.nationalID !== ""
        ...
        <Row className=" d-flex align-items-center mb-3">
          ...
          <Form.Group as={Col} sm={12} md={4} controlId="formGridNationalID" className=''>
            ...
            <Form.Control value={formInput.nationalID.replace(/[^0-9]/ig, "")} required name="nationalID"
              onWheel={(e) => e.target.blur()} type="number" placeholder="رقم الهوية"
              onChange={(e) => handleChange(e)} className={`${formMessages?.nationality_number ? 'uLT-bd-f-danger-sA' : 'uLT-bd-f-platinum-sA'} uLT-f-radius-sB cLT-main-text fLT-Regular-sB`} />
            {formMessages?.nationality_number && <p className='position-absolute text-nowrap mb-0 fLT-Regular-sA cLT-danger-text pt-2 px-2'>{formMessages?.nationality_number}</p>}
          </Form.Group>
          {/* Gender */}
