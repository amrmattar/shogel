...
const OrdersPage = () => {
    ...
    const listOfUsersOrder = useMemo(() => {
        return userOfferPrice._GET_AllOrderList(10, true, param.num, getSearchKey.searchStatus, getSearchKey?.searchKey).then(res => {
    ...
    useEffect(() => {
        const timeout = setTimeout(() => { return listOfUsersOrder }, 1200);
    ...
    useEffect(() => {
        const timeout = setTimeout(() => {
            if (!userOfferDetatils?.pagination) { return handleAdvsPagination }
        }, 1000);
        return clearTimeout(timeout)
    ...
    if (!userOfferDetatils) return <div className='d-flex justify-content-center align-items-center w-100 ' style={{ height: '100vh' }}>
                ...
                <div className='d-flex flex-column w-100'>
                    <div className="cLT-white-bg p-3 ">
                        {userOfferDetatils?.map((offer, ix) => {
                            return <NavLink className="card px-3 pt-3 mb-3 uLT-f-radius-sB uLT-list-style" to={`/orders/order-details/${offer.id}`} key={ix} > <OrderListCardComponent roll={vistorUser}
                                amentiesSelector={<AmentiesShared orderData={offer} amentiesWithIcon="orderAmenties" />}
