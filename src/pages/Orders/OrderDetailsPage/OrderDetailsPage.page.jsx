...
const OrderDetailsPage = () => {
    ...
    const [userStatus, messages, userRole, userPermission] = useSelector((state) => [state.authentication.loggedIn, state.messages, state.userRole.userRole, state.userData])
    ...
    const offerDataById = useCallback(() => {
        return userOfferPrice._GET_OffersPriceTaskByID(param.id).then(res => {
    ...
    const [getOfferData, setRequestData] = useState({
        offerPrice: '', description: '', offerTime: '',
    })
    ...
    const sendOffer = (e) => {
        ...
        dispatch(getMessages({ messages: 'جاري إرســـال طلب عرضك', messageType: 'warning', messageClick: true }))
        const RequestOfferData = {
            price: getOfferData.offerPrice, time: getOfferData.offerTime,
            description: getOfferData.description, task_id: offerPriceTaskData?.id
        }
        userOfferRequest._POST_RequestOfOffer(RequestOfferData).then(res => {
            dispatch(getMessages({ messages: res.data.message, messageType: 'success', messageClick: true }))
            setOfferRequest(false)
            setRefreshTasks(true)
            setRequestData(getOfferData => ({ ...getOfferData, description: '', offerPrice: '', offerTime: '' }))
        }).catch(err => {
    ...
    if (!offerPriceTaskData) return <div className='d-flex justify-content-center align-items-center w-100 ' style={{ height: '100vh' }}>
    ...
    return (
        <>
            ...
            <div className='d-flex flex-column flex-lg-row px-3 gap-3 px-lg-0 container-lg'>
                <div>
                    <SideOrderDetailsPage offerRefresh={setRefreshTasks} data={offerPriceTaskData} isUser={activeUserID} />
                </div>
                <div className='w-100'>
                    {/* Order Details [Holder] */}
                    <div className="">
                        <OrderListCardComponent amentiesSelector={<AmentiesShared amenties="amenties" orderData={offerPriceTaskData} />}
                    ...
                    {userStatus ?
                        <>
                            ...
                            {havePermission && !activeUserID && !taskBoolenOfferData && offerPriceTaskData?.status?.name !== 'in progress' ?
                                <>
                                    <PageTitle smallUnderTitle=" " title="أضف عرضك الآن" />
                                    <OfferRequestForm errMessage={errMessage} handleCLick={sendOffer}
                                        handleChange={getOfferDataValue} offerIsRequest={offerRequest} formValue={getOfferData}
                                    />
                                </> : false
                            }
                                ...
                                <>
                                    {offerPriceTaskData?.offer?.map((offerList, idx) => {
                                        return <div className="" key={idx}>
                                            <FlancerEmployedOfferComponent setLoading={refreshTasks} data={offerList} isMyTask={activeUserID} isMyOffer={taskOfferData} isComment={setRefreshTasks} />
                                        </div>
                                    })}
                                </>
                                :
                                <div className="d-flex flex-column justify-content-center align-items-center w-100 " >
                                    <div className="imLT-main-logo uLT-img-contain uLT-f-radius-sB img-fluid uLT-f-radius-sB" style={{ width: '150px', height: '100px' }}></div>
                                    <p className='mb-0 fLT-Bold-sB cLT-gray-text'>لا يوجد عـــــروض</p>
                                </div>
                            }
                        ...
                        <div className="">
                            ...
                            <div className="d-flex justify-content-start align-items-center cLT-platinum-bg uLT-f-radius-sB p-4">
                                <p className="mb-0 cLT-support2-text fLT-Bold-sA ">لا يمكنك ان تقدم عرضا لهذا الطلب تاكد من <button
                                    onClick={() => setNextClick(!nextClick)}
                                    className='btn px-0 cLT-secondary-text uLT-click' style={{ color: '#1EAAAD' }} >تسجيل الدخول</button> اولا</p>
                            </div>
