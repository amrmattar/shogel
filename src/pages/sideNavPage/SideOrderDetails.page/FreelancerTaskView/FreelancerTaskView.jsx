...
const FreelancerTaskView = ({ data, setLoading }) => {
    ...
    const [jobTaskDone, setJobTaskDone] = useState({ description: '', comment: '' })
    ...
    const handleSendTaskFile = (e) => {
        ...
        const reviewData = {
            user_id: data?.user?.id, task_id: data?.id,
            review: rateNumber ? rateNumber : 1, comment: jobTaskDone?.comment
        }
        // Get Files Upload [Image,Videos] And Decription
        file.images?.map((image, idx) => { return media.append(`done`, image) });
        file.videos?.map((video, idx) => { return media.append(`done`, video) });
        media.set('comment', jobTaskDone.description)
        userOfferPrice._POST_FileDoneByfreelancer(data?.id, media).then(res => {
    ...
    return (
        <div className='d-flex flex-column gap-3'>
            ...
            <div className="d-flex flex-column gap-3 position-relative mt-4">
                ...
                <textarea style={{ minHeight: '120px', resize: 'unset' }}
                    className=" form-control uLT-f-radius-sB p-3 uLT-bd-f-platinum-sA fLT-Regular-sB cLT-main-text" rows="1" placeholder="صف مشروعك هنا"
                    name='description' maxLength={500} value={jobTaskDone?.description}
                    onChange={handleChange}>
                </textarea>
            </div>
            {/* Upload Files [Holder] */}
            <div className="LT-upload-advs~ d-flex flex-column">
                <Upload inputRef={inputRef} isDrop={fileHandler} targetClick={filePicker} fileArr={filenames} handleDelete={handleDelete} changeStylrToAnotherComponnet={'taskComponent'}
            ...
            <div className="d-flex flex-column gap-2 position-relative mt-4">
                <Form.Label className="form-label fLT-Bold-sA cLT-support2-text m-0"> تعليقك عن صاحب الطلب</Form.Label>
                <textarea style={{ minHeight: '120px', resize: 'unset' }}
                    className=" form-control uLT-f-radius-sB p-3 uLT-bd-f-platinum-sA fLT-Regular-sB cLT-main-text" rows="1" placeholder="صف تعليقك"
                    name='comment' value={jobTaskDone?.comment} maxLength={500}
                    onChange={handleChange}>
                </textarea>
            </div>
            ...
            <div className="d-flex justify-content-end  align-items-left">
                <div className="shadow uLT-f-radius-sB">
                    <ButtonShare
                        ...
                        textClasses="px-4 cLT-white-text fLT-Regular-sC" innerText=" تسليم الطلب" />
                </div>
            </div>
        </div>
    )
