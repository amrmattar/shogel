...
const SideNavFlancerProfilePages = ({ data }) => {
    ...
    const handleClick = (id, userInfo) => {
        if (id === 'whatsApp') {
            window.open(`https://api.whatsapp.com/send/?phone=966592031195&text=${'start Chat'}`);
        }
        if (id === 'email') {
            window.open(`https://mail.google.com/mail/mu/mp/159/#co/to=${userInfo?.email}`);
        }
        if (id === 'mobile') {
        ...
        <div className='cLT-white-bg p-4 d-flex flex-column gap-3 ' >
                ...
                <div className="d-grid gap-3 uLT-bd-b-platinum-sA pb-4">
                    ...
                    <div className="d-flex align-items-center gap-3">
                        {communicated.map((communicate, ix) => {
                            return <div key={ix} onClick={() => handleClick(communicate.id, data)} >
                                <FlancerCommunicatedComponent iconName={communicate.iconName} iconID={communicate.id} />
                            </div>
                        })}
                        <NavLink to={`/freelancer-offer/${data?.id}`} className={'uLT-list-style'} >
                            <div className="shadow  uLT-f-radius-sB">
                                <ButtonShare btnClasses="cLT-secondary-bg py-2 px-4  d-flex align-items-center gap-2 uLT-f-radius-sB" textClasses={`fLT-Regular-sB px-1 cLT-white-text `} innerText="شغلني" iconName={'iLT-work-case-white'} />
                            </div>
                        </NavLink>
                    </div>
                </div>
            ...
            <div className="d-grid gap-3 uLT-bd-b-platinum-sA pb-4">
                ...
                <div className="d-flex gap-3 flex-wrap">
                    {data?.category?.map(category => {
                        return <AmentiesShared key={category.id} amenties={'areasOfCompetence'} areasOfCompetenceData={category?.name} />
                    })}
                </div>
            </div>
            ...
            <div className="d-grid gap-3 uLT-bd-b-platinum-sA pb-4">
                ...
                {data?.skill?.map((skill, ix) => {
                    return <div key={ix} className="d-flex justify-content-between">
                        <FlancerCoursesComponent innerText={skill.skill} />
                        <PerformanceShared defaultNumber={skill.level.level} status={true} />
                    </div>
                })
                }
                   </Fragment> : 
                    <p className='mb-0 fLT-Bold-sA cLT-gray-text'>لا يوجد شهادات ودورات</p>
            }
            </div>
            ...
            <div className="d-grid gap-3 uLT-bd-b-platinum-sA pb-4">
                ...
                {data?.language?.map((lang, ix) => {
                    return <div key={ix} className="d-flex justify-content-between">
                        ...
                        <PerformanceShared defaultNumber={lang?.level?.level} status={true} />
                    </div>
                })
            }
            </Fragment> : 
            ...
            <div className="uLT-bd-b-platinum-sA pb-4">
                ...
                <div className="d-flex align-items-center flex-wrap gap-3">
                    {data?.reverse?.map((document, ix) => {
                        return <div key={ix}>
                            <FlancerDocumentationComponent status={document.status} documentName={document.title} />
                        </div>
                    })}
                </div>
            </div>
