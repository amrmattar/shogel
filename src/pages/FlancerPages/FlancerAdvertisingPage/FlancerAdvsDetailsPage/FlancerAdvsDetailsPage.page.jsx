...
const FlancerAdvsDetailsPage = () => {
    const [vistorUser, messages] = useSelector((state) => [state.authentication.loggedIn, state.messages]);
    ...
    const getAdvsById = useMemo(() => {
        return advertisingLists._GET_MyAdvsById(param?.id).then(res => {
    ...
    const getAdvsByCategory = useMemo(() => {
        if (advsCategory) {
            return advertisingLists._GET_AdvertisingByCategory(4, true, advsCategory, advsDataById?.id).then(res => {
    ...
    const startChat = () => {
        window.open(`https://api.whatsapp.com/send/?phone=${advsDataById?.user?.mobile}&text=${'https://shogol.sa/'}`);
    }
    const sendEmail = () => {
        // window.open(`https://mail.google.com/mail/?view=cm&fs=1&tf=1&to=${advsDataById?.user?.email}`);
        window.open(`https://mail.google.com/mail/mu/mp/159/#co/to=${advsDataById?.user?.email}`);
    }
    ...
    const addFavourite = () => {
        AdvertisingFav._POST_AdvsFavourite(advsDataById?.id).then(res => {
    ...
    if (!advsDataById) return <div> <p className='mb-0 fLT-Bold-sD cLT-gray-text text-center'>Loading</p> </div>
    return (
        <div>
            ...
            <div className="mb-3 container-md px-2 px-sm-3 px-md-4" >
                ...
                <div className="LT-advsDetails-grid cLT-white-bg uLT-f-radius-sB ">
                    ...
                    <div className="LT-advsDetails-image position-relative">
                        <img src={advsDataById?.user?.avatar} alt="myPic" className={'w-100 h-100 iLT-sF uLT-f-radius'} />
                    ...
                    <div className="LT-advsDetails-list-card">
                        ...
                        <div className="d-flex align-items-center justify-content-between">
                            <div className="d-flex justify-content-between pe-sm-2">
                                <p className="m-0 ps-2 ps-sm-3 card-text fLT-Bold-sm-sA cLT-support2-text text-nowrap">{advsDataById?.user?.fullname}</p>
                                <img src={advsDataById?.nationality?.logo ? advsDataById?.nationality?.logo : defautFlag} alt="" className={' iLT-sC '} />
                            </div>
                            {/* List Card Rate */}
                            <div className='d-flex justify-content-between align-items-start'>
                                ...
                                <div className="LT-performance-rate-holder">
                                    ...
                                    <div className="d-flex align-items-center LT-rate-font-size">
                                        <p className="m-0 card-text cLT-support2-text ">({advsDataById?.user?.rate?.count})</p>
                                        <p className="m-0 card-text cLT-support2-text ">{advsDataById?.user?.rate?.rate}</p>
                        ...
                        {/* Card title */}
                        <div className="">
                            <p className="m-0 LT-advsDetails-title cLT-support2-text">{advsDataById?.name}</p>
                        </div>
                    </div>
                    {/* Card Amenties */}
            ...
            <div className="container-md mb-4  px-2 px-sm-3 px-md-4">
                ...
                <div className=''>
                    <p className={`m-0 fLT-Regular-sB cLT-smoke-text ${showDescription === true ? '' : 'text-ellipsis3'}  ${location.pathname === `/advertising/advertise-details/${param?.id}` ? '' : 'text-ellipsis3'}`} dangerouslySetInnerHTML={{ __html: advsDataById?.description !== 'undefined' ? advsDataById?.description : 'لايوجد وصف للإعلان' }}></p>
                </div>
                <div className="d-flex align-items-end p-0">
                    {location.pathname === `/advertising/advertise-details/${param?.id}` &&
            ...
            <div className="">
                <FlancerCertificatesComponent certificatesData={advsDataById?.document} />
            </div>
            {vistorUser &&
                <div className="container-md px-4 mt-5">
                    <div className="d-flex flex-wrap gap-3 justify-content-between align-items-center position-relative">
                        <div className="d-flex align-items-center gap-3 flex-wrap ">
                            <div onClick={() => sendEmail()} className="uLT-advs-contact uLT-click d-flex justify-content-center align-items-center uLT-bd-f-secondary-sA uLT-f-radius-sB p-2">
                                <i className={`iLT-flancer-email uLT-img-contain iLT-sC`}></i>
                            </div>
                            <div onClick={() => copyPhoneNumber(advsDataById?.user?.mobile)} className="uLT-advs-contact uLT-click d-flex justify-content-center align-items-center uLT-bd-f-secondary-sA uLT-f-radius-sB p-2">
                                <i className={`iLT-flancer-mobile uLT-img-contain iLT-sC`}></i>
                                <Toast className='s' onClose={() => setCopyMobile(false)} show={mobileCopy} delay={2000} autohide>
                                    <Toast.Body>The Phone Number Is copied</Toast.Body>
                                </Toast>
                            </div>
                            <div onClick={() => startChat()} className="uLT-advs-contact uLT-click d-flex justify-content-center align-items-center uLT-bd-f-secondary-sA uLT-f-radius-sB p-2">
                                <i className={`iLT-flancer-whatsApp uLT-img-contain iLT-sC`}></i>
                            </div>

                            <div onClick={() => addFavourite()} className="uLT-advs-contact uLT-click d-flex justify-content-center align-items-center uLT-bd-f-secondary-sA uLT-f-radius-sB p-2">
                                <i className={`${advsDataById.is_favourite !== 0 ? 'iLT-flancer-red-heart' : 'iLT-flancer-heart'} uLT-img-contain iLT-sC`}></i>
                            </div>
                            <CopyToClipboard text={`${window.location.href}`}>
                                <div onClick={() => startShare()} className="uLT-advs-contact uLT-click  d-flex justify-content-center align-items-center uLT-bd-f-secondary-sA uLT-f-radius-sB p-2">
                                    <i className={`iLT-flancer-share uLT-img-contain iLT-sC`}></i>
                                    <Toast className='s' onClose={() => setCopyUrl(false)} show={urlCopy} delay={2000} autohide>
                        ...
                        <div className="d-flex align-items-center gap-3">
                            <div className="uLT-advs-contact uLT-click d-flex justify-content-center align-items-center uLT-bd-f-platinum-sA uLT-f-radius-sB p-2 ">
                                <i className={`   iLT-flancer-report uLT-img-contain iLT-sC `}></i>
                            </div>
                            <p className='m-0 fLT-Regular-sB cLT-smoke-text'>  ارسال  بلاغ  </p>
                        </div>
                    </div>
                </div>
            }
            <div className="container-md d-flex flex-column px-2 px-sm-3 px-md-4 flex-wrap">
                ...
                <div className="LT-list-Of-AdvsCategory ">
                    ...
                    {advsDataByCategory?.map((advs, ix) => {
                        return <NavLink onClick={() => scrollToTop()} className="uLT-list-style" to={`/advertising/advertise-details/${advs?.id}`} key={ix} > <FlancerAdvsGridCards data={advs} />  </NavLink>
                    })}
                </div>
            </div>
        </div>
