...
const FlancerProfilePage = ({ data }) => {
    ...
    const getMyAdvs = useMemo(() => {
        return advertisingLists._GET_SelectUserAdvsOffer(5, true, 1, param.id).then(res => {
    ...
    return (
        <div className="container-lg px-0 overflow-hidden cLT-white-bg ">
            ...
            <div className="">
                ...
                <FlancerRate rateNumber={parseInt(data?.rate?.rate ? data?.rate?.rate : 0)} />
            </div>
            {/* User Reviews */}
            <div className=" ">
                ...
                <div className="overflow-hidden ">
                    <div className="LT-user-profile-grid  overflow-auto d-flex">
                        {Array(5).fill().map((elem, ix) => { return <div className="" key={ix}> <FlancerCommentAndReviews reviews={data?.rate?.rate} /> </div> })}
                    </div>
                </div>
            </div>
            {/* User Certificates */}
            <div className="  overflow-hidden">
                <PageTitle title="صور الاعمال والشهادات" />
                    ...
                    <div className="imLT-main-logo uLT-img-contain uLT-f-radius-sB img-fluid uLT-f-radius-sB" style={{ width: '120px', height: '65px' }}></div>
                    <p className='mb-0 fLT-Bold-sC cLT-gray-text'>لا يوجد صور الاعمال والشهادات</p>
                    </div>
                }
            </div>
            {/* User Advertise */}
            <div className=" pb-4 ">
                ...
                <div className="overflow-hidden ">
                    ...
                    <div className="LT-user-profile-grid overflow-auto d-flex">
                        {myAdvs?.map((advs, idx) => {
                            return <NavLink className="uLT-list-style" to={`/advertising/advertise-details/${advs.id}`} key={idx} >
                    ...
                    <div className="d-flex flex-column justify-content-center align-items-center w-100  " style={{ height: '100% ' }}>
                    <div className="imLT-main-logo uLT-img-contain uLT-f-radius-sB img-fluid uLT-f-radius-sB" style={{ width: '120px', height: '65px' }}></div>
                    <p className='mb-0 fLT-Bold-sC cLT-gray-text'>لا يوجد اعلانــــات</p>
                </div>
                    }
                </div>
            </div>
