...
const FlancerMyEditAccountPage = ({ handleClick, personalData , updateLoading, setCheck, onClick, fileUploads}) => {
    //TODO Data from Reducers
    const [coreData, userRole, messages] = useSelector((state) => [state.coreData, state.userRole.userRole, state.messages ])
    ...
    return (
        <div className='d-flex flex-column gap-4 px-0'>
            <>
                ...
                <FlancerEditPersonalAccountComponent data={coreData} userProfileData={personalData} />
            </>
            {/* About Me [Section] */}
            <>
                {/* About Me [Title] */}
                ...
                {/* About Me [Component] */}
                <FlancerAboutComponent about={'editAbout'} data={personalData?.description} />
            </>
            {/* My Location [Section] */}
            <>
                {/* My Location [Title] */}
                ...
                <FlancerEditLocationComponent screenCol="6" claases={'mb-3'} userProfileLocation={userLocation ? userLocation : false}  />
            </>
            {/* My Tags [Section] */}
            {userRole !== '2' &&
            <>
                ...
                {/* My Tags [Component] */}
                <FlancerEditTagsComponent type={'Register'}
                    tags={coreData?.category}
                    userProfileTags={personalData?.category}
                    tagDescription={'هناك حقيقة مثبتة منذ زمن طويل وهي أن المحتوى المقروء لصفحة ما سيلهي القارئ عن التركيز على الشكل الخارجي للنص أو شكل توضع الفقرات في الصفحة التي يقرأها.'} />
            </> 
            }
            ...
            <>
                ...
                <FlancerBusinessPhotosComponent data={personalData}  fileUpload={fileUploads}  handleClick={onClick}/>
            </>
            ...
            <>
                ...
                <FlancerOtherJobSitesComponent socialSite={coreData?.social} data={personalData?.social} checkupdate={setCheck}/>
            </>
            {/* Other Job Sites Save [Button] */}
            <div className="shadow uLT-f-radius-sB pb-4">
                <ButtonShare loading={updateLoading} onClick={handleClick} innerText={'حفظ'} btnClasses={'cLT-secondary-bg px-4 py-3 uLT-f-radius-sB  '} textClasses={'px-4 fLT-Regular-sD cLT-white-text'} />
            </div>
        </div>
    )
}
