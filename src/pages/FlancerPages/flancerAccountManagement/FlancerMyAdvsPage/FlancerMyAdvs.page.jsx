...
const FlancerMyAdvsPage = () => {
    ...
    const myAdvsList = useMemo(() => {
        ...
        return advertisingLists._GET_MyAdvsOffer(10, true, param.num, advsStatusID).then(res => {
    ...
    useEffect(() => {
        const timeout = setTimeout(() => {
            if (!myAdvsData?.pagination) { return handleAdvsPagination }
        }, 1200);
        return clearTimeout(timeout)
    ...
    const [adStatus, setStatus] = useState({ cancel_Status: Number, update_Status: Number, active_Status: Number })
    const handleAdvsStatus = useMemo(() => {
        return myAdvsData?.data?.map((advsStatus) => {
            ...
            if (advsStatus.status.id === 3) {
                setStatus(prevVal => ({ ...prevVal, active_Status: advsStatus.status.id }))
            }
            // Advertising Status Type [Cancel] ========= [ID: 7]
            else if (advsStatus.status.id === 7) {
                setStatus(prevVal => ({ ...prevVal, cancel_Status: advsStatus.status.id }))
            }
            // Advertising Status Type [Expired Time Or Date] ========= [ID: 9]
            else if (advsStatus.status.id === 9) {
                setStatus(prevVal => ({ ...prevVal, update_Status: advsStatus.status.id }))
            }
        })
    }, [myAdvsData?.data])
    useEffect(() => {
        if (!adStatus) { return handleAdvsStatus }
    }, [adStatus])
    // TODO GET From API Response ==> Advertising Status ID 
    // ? ------------------[[[END Block]]]-----------------
    ...
    const advsData = {
        myAllAdvsList: <AllAdvertisingControllerComponent allAdvsRefresh={setRefresh} loading={advsResponseLoading} onChange={getPageNumber} allStatus={advsStatusID} data={myAdvsData?.data} />,
        myActiveAdvsList: <MyActiveAdvertisingComponent activeRefesh={setRefresh} activeStatus={adStatus.active_Status} loading={advsResponseLoading} onChange={getPageNumber} data={myAdvsData?.data} />,
        myCancelAdsList: <MyCancelAdvsListComponent cancelStatus={adStatus.cancel_Status} loading={advsResponseLoading} onChange={getPageNumber} data={myAdvsData?.data} />,
        myExpiredAdsList: <MyExpiredAdvertisingComponent expiresRefesh={setRefresh} updateStatus={adStatus?.update_Status} loading={advsResponseLoading} onChange={getPageNumber} data={myAdvsData?.data} />,
    }

    return (
        <div className='d-flex flex-column align-items-center justify-content-between gap-3 h-100' >
            <div className="px-3 w-100"  >
                <FlancerAdvsControllerShared onClick={(e) => handleChangeTabs(e)} children={advsData} />
            </div>
