...
const Home = () => {
            ...
            if(homeData?.section === '3' || homeData?.section === '4') {
                switch (homeData?.key) {
                    case 'home_section_3_image':
                        return  setSectionThreeData(sectionThreeData => ({...sectionThreeData, image: homeData.value}))
                        case 'home_section_4_url':
                            return  setSectionFourData(sectionFourData => ({...sectionFourData, url: homeData.value}))
                    default:
                        return false
                }
            }
    ...
    const listOfUsersAdvs = useMemo(() => {
        return advertisingLists._GET_AllAdvsOffer(4,false,1,false,'').then(res => {
    ...
    useEffect(() => {
        const timeout = setTimeout(() => { return listOfUsersAdvs }, 300);
            ...
            <section className="container-md px-0 text-center ">
                ...
                <section className="LT-card-grid-holder ">
                    ...
                    {sectionThreeData?.image?.map((category, ix) => {
                        return <CategoryListComponent  ix={ix} iconName={category.iconName} categoryName={category.categoryName} key={ix} />
                    })} 
                </section>
                   : 
                   <div className="d-flex flex-column justify-content-center align-items-center w-100"  >
                    ...
                    <p className='mb-0 fLT-Bold-sD cLT-gray-text'>لا يتوافر اشغال حاليا</p>
                </div>
                }
            </section>
            <Divider spaceSize="py-5" />
            <section className="container-md d-flex flex-column px-2 px-sm-3 px-md-4 ">
                ...
                <section className="LT-card-grid-holder">
                    {userAdvsDetatils?.map((elem, idx) => {
                       return <NavLink className="uLT-list-style" to={`/advertising/advertise-details/${elem?.id}`} key={idx} > <FlancerAdvsGridCards data={elem} key={idx} />  </NavLink>
                    })}
                </section>
                ...
                <section className="d-flex w-100 px-0 justify-content-center align-items-center ">
                    <NavLink to={sectionFourData?.url} className="shadow uLT-f-radius-sB">
                        <ButtonShare btnClasses="cLT-secondary-bg px-4 uLT-f-radius-sB py-3" textClasses="px-4 cLT-white-text fLT-Regular-sC" innerText="عرض الكل" />
                    </NavLink>
                </section>
            </section>
            <Divider spaceSize="py-5" />
