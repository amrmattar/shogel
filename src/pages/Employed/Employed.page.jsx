...
const Employed = () => {
    ...
    useEffect(() => {
        let cancel = false;
        freelancersListProfile._GET_FreelancersListProfile(10, true, param.num, getSearchKey.searchStatus, getSearchKey?.searchKey).then(res => {
            if (cancel) return;
    ...
    useEffect(() => {
        const timeout = setTimeout(() => {
            if (!flancersList?.pagination) { return handleAdvsPagination }
        }, 1000);
        return clearTimeout(timeout)
   ...
   if (!flancersList?.data) return <div className='d-flex justify-content-center align-items-center w-100 ' style={{ height: '100vh' }}>
                ...
                <div className='d-flex flex-column w-100'>
                    <div className="cLT-white-bg p-3 ">
                                {flancersList?.data?.map((list, idx) => {
                                    return <NavLink className="uLT-list-style" to={`/employed/freelancer-profile/${list?.id}`} key={idx}> <FlancerEmployedListCard data={list} />  </NavLink>
                                })}
                    </div>
                     {/* Pagination [Holder] */}
