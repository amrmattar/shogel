...
const LoginPage = ({ open, setLoginOpen, setSignupOpen, setMobileOpen, forgetPass }) => {
        ...
        LoginServices._POST_LoginData(dataWithToken)
            ...
            .catch((err) => {
                ...
                if (stateLoginData?.email !== '' && stateLoginData?.password !== '') {
                    ...
                    dispatch(getMessages({ messages: err.response.data.message.password[0], messageType: 'error', messageClick: true }))
        ...
        <>
            ...
            <Dialog aria-labelledby="simple-dialog-title1" open={open ? open : false} onClose={handleClose}>
                <Form onSubmit={(e) => formSubmit(e)} >
                    <LoginComponent loginCheck={loginCheck} forgetPassword={() => setLoginOpen(false)} />
                    <div className="d-flex align-items-center justify-content-center py-3 gap-1" style={{ paddingBottom: '3rem' }}>
                        ...
                        {/* <Button onClick={switchSignup} className='px-0'>
                          ...
                          <NavLink to={'/register'} onClick={() => setLoginOpen(false)} className="uLT-list-style fLT-Bold-sm-sA cLT-secondary-text">
                             إنشاء حساب جديد
                        </NavLink>
                    </div>
                </Form>
