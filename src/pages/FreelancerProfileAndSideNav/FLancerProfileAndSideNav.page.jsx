...
const FLancerProfileAndSideNavPage = () => {
    ...
    useEffect(() => {
        if (isResult === '') { return getCoreData }
    }, [isResult, getCoreData])
    // Navigate If Data Not Found
    if (!isResult) return <div className='d-flex justify-content-center w-100 h-100'><div className=" spinner"></div></div>
    return (
        <>
        <Outlet/>
            <div className='d-flex flex-column px-0 gap-3 container-fluid '>
                {/* Main Back Ground  */}
                <div className="container-fluid py-5 px-4 cLT-main-bg" style={{ height: '217px' }}></div>
                <div className="d-flex flex-column flex-lg-row px-0 px-sm-3 gap-3 px-lg-0 container-lg">
                    <div className="LT-sidenav-media-screen" >
                        {/* <SideNav User Personal Data /> */}                                                            {/* <SideNav User Skills Data /> */}
                        <AsideFreelancerPage userDataByParam={isResult} setMarginTop={-70} setMarginBottom={10} selector={<SideNavFlancerProfilePages data={isResult} />} />
                    </div>
