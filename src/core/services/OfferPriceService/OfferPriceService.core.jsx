...
function _GET_AllOrderList(page, pagination, pageNum, searchType, keyName) {
  return API.get(
    `task/list?perPage=${page}&pagination=${pagination}&page=${pageNum}&search=${searchType}&name=${keyName}`
  );
}
...
function _GET_MyOfferTaskLists(page, pagination, pageNum, status) {
  ...
  return API.get(
    `task/my_offer?perPage=${page}&pagination=${pagination}&page=${pageNum}&status_id=${status}`,
    requestOption
  );
}
// _GET_MyOfferSpecialLists
function _GET_MyOfferSpecialLists(page, pagination, pageNum, status) {
  ...
  return API.get(
    `task/my_list_special?perPage=${page}&pagination=${pagination}&page=${pageNum}&status_id=${status}`,
    requestOption
  );
}
// _GET_MyOfferGeneralLists
function _GET_MyOfferGeneralLists(page, pagination, pageNum, status) {
  ...
  return API.get(
    `task/my_list?perPage=${page}&pagination=${pagination}&page=${pageNum}&status_id=${status}`,
    requestOption
  );
}
