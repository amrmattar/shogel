    ...
    const getAllOfferTasksList = useCallback(
        () => {
            if (type === 'priceListOffer') {
                userOfferPrice._GET_MyOfferGeneralLists(10, true, param.num, offerStatus !== null ? offerStatus : '').then(res => {
            ...
            {myOfferList?.length !== 0 ?
                <>
                    {myOfferList?.map((data, idx) => {
                        return <NavLink className="card px-3 pt-3 mb-3 uLT-f-radius-sB uLT-list-style" to={`/orders/order-details/${data?.id}`} key={idx}>
                            <OrderListCardComponent key={idx}
                                orderTitle={data?.name}
                                orderStatus={data?.status?.name}
                                orderDescription={data?.description}
                                amentiesSelector={<AmentiesShared orderData={data} amentiesWithIcon="orderAmenties" />} />
                        </NavLink>
                    })}
                </> :
