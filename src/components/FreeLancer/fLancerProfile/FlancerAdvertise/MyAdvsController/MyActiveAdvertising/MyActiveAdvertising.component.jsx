...
const MyActiveAdvertisingComponent = ({ data, loading, btnLoad, activeStatus, activeRefesh }) => {
    ...
    return (
        <>
            ...
            <CancelationShared cancelType={'advs'} refe={refs} cancelRefesh={activeRefesh} reason={cancelIsReason} advsID={canceladvsID} id={'openModal'} dataTarget={'#myAdvertising'} modalId={'myAdvertising'} />
            {activeStatus === 3 && data?.length !== 0 ?
                <>
                    {data?.map((adData, idx) => {
                        return <div onClick={(e) => { routeAdvsDetails(e, adData.id) }} className={'uLT-click'} key={idx}>
                            <div key={idx} className="LT-myAdvs-grid uLT-f-radius-sB uLT-bd-f-platinum-sA mb-3">
                                {/* List Card [Image] */}
