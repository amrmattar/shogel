...
const AllAdvertisingControllerComponent = ({ data, loading, allAdvsRefresh }) => {
    ...
    const routeAllAdvsDetails = (e, adsId) => {
        ...
        if (btnTarget !== 'حذف' && btnTarget !== 'ايقاف' && btnTarget !== 'تعديل' && btnTarget !== 'تحديث') {
    ...
    return (
        <>
            ...
            {data?.length !== 0 ?
                <>
                    <CancelationShared cancelType={'advs'} cancelRefesh={allAdvsRefresh} refe={refes} reason={cancelReason} advsID={canceladvsID} id={'openModal'} dataTarget={'#allAdvertising'} modalId={'allAdvertising'} />
                    {data?.map((adData, idx) => {
                        return <div key={idx} onClick={(e) => routeAllAdvsDetails(e, adData.id)} className="uLT-click LT-myAdvs-grid uLT-f-radius-sB uLT-bd-f-platinum-sA mb-3">
                            {/* List Card [Image] */}
                            ...
                            <div className="LT-myAdvs-details">
                                ...
                                <div className="LT-myAdvs-title  ">
                                    <FlancerDescriptionTitleComponent fontSize={'fLT-Bold-sm-sB'} descriptionTitle={adData?.name} />
                                    <FlancerDescriptionBodyComponent fontSize={'text-ellipsis2 fLT-Regular-sB '} descriptionBody={adData?.description} />
                                </div>
                                {/* Card Amenties */}
