...
const MyExpiredAdvertisingComponent = ({
  data,
  loading,
  updateStatus,
  expiresRefesh,
  ...
  const handleUpdateAdvs = (id) => {
    dispatch(
      getMessages({
        messages: "جارى ارسال طلب التحديث ",
        messageType: "warning",
        messageClick: true,
      })
    );
    advertisingLists
      ._POST_UpdateAdvertising(id)
      .then((res) => {
        expiresRefesh(true);
        dispatch(
          getMessages({
          ...
          {data?.map((adData, idx) => {
            return (
              <div
                key={idx}
                className="uLT-click"
                onClick={(e) => routeExpireAdvsDetails(e, adData.id)}
              >
