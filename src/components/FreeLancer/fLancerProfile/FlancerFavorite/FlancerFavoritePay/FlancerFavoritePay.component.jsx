...
const FlancerFavoritePayComponent = () => {
  ...
  const favoriteList = useMemo(() => {
    AdvertisingFav._GET_AdvsFavourite().then((res) => {
      setAllFavoriteList(res.data.data);
    });
  }, []);
  useEffect(() => {
          ...
          {allFavoriteList?.map((favList, ix) => {
            return (
              <NavLink
                ...
                to={`/advertising/advertise-details/${favList.id}`}
                key={ix}
              >
                {" "}
                <FlancerAdvsListCardComponent data={favList} />{" "}
              </NavLink>
            );
          })}
        </>
