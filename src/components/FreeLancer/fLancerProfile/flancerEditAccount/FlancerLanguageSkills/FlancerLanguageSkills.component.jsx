...
import { deleteLanguage, getLanguage } from '../../../../../core/redux/reducers/LanguageSkillsReducer.core'
...
const FlancerLanguageSkillsComponent = ({ langRequired, langRef }) => {
    ...
    const langHandleClick = (e) => {
        ...
        if (languageRef.current?.value === '') {
            ...
            dispatch(getLanguage({ skill: languageVal, id: Date.now().toString(), level_id: lanSkillsData?.fLanguageRate, type: 'language' }))
            setLanguageVal('')
            setType(true)
        }
    }
