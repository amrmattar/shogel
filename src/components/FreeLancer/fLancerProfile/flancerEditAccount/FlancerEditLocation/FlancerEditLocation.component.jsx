...
import { getcitiesID, getCountryID, getStateID, getAreaID } from '../../../../../core/redux/reducers/RegisterReducer/RegisterLocationID.core';
...
const FlancerEditLocationComponent = ({ userProfileLocation }) => {
    ...
    const getCoreData = useMemo(() => {
        ...
        return RegisterServices.GET_RegisterData(modal, userProfileLocation?.country?.id, userProfileLocation?.city?.id).then(res => {
    ...
    function CountrySelect(isProps = {}, ref) {
        return <Select placeholder="البلد"
            ...
            defaultInputValue={countryName ? countryName : isProps.isProps.value ? isProps.isProps.value : 'loading'}
    ...
    function CitySelect(isProps = {}, ref) {
        return <Select placeholder="المنطقة"
            ref={refe}
            options={isProps.isProps.location}
            defaultInputValue={cityName ? cityName : isProps.isProps.value ? isProps.isProps.value : 'loading'}
    ...
    function StateSelect(isProps = {}, ref) {
        return <Select placeholder="المدينة"
            ref={refe}
            options={isProps.isProps.location}
            defaultInputValue={stateName ? stateName : isProps.isProps.value ? isProps.isProps.value : 'loading'}
            onChange={fetchState}
    ...
    function AreaSelect(isProps = {}, ref) {
        return <Select placeholder="الحى"
            ref={refe}
            options={isProps.isProps.location}
            defaultInputValue={areaName ? areaName : isProps.isProps.value === null ?  'loading' : isProps.isProps.value }
            onChange={fetchArea}
            getOptionLabel={state => state?.name} getOptionValue={state => state?.id} />;
    }
    ...
    return (
       ...
       <Form.Group as={Col} md={6} className="'mb-3  " >
         ...
         <div className={`uLT-bd-f-platinum-sA uLT-f-radius-sB cLT-main-text fLT-Regular-sB LT-edit-account-input mb-3 mb-md-0`} >
           <CountrySelect isProps={{ value: userProfileLocation?.country?.name, location: getAllCountryFromResponse?.country }} ref={refe} />
         </div>
       </Form.Group>
       {/* City [Section] */}
       <Form.Group as={Col} md={6} className="'mb-3 mb-md-0">
         ...
         <div className={`uLT-bd-f-platinum-sA uLT-f-radius-sB cLT-main-text fLT-Regular-sB `}>
           <CitySelect isProps={{ value: userProfileLocation?.city?.name, location: getAllCountryFromResponse?.city }} ref={refe} />
         </div>
       </Form.Group>
     </Row>
     <Row>
       ...
       <Form.Group as={Col} md={6} className="'mb-3 mb-md-0">
         {/* State [Option]  */}
         <div className={`uLT-bd-f-platinum-sA uLT-f-radius-sB cLT-main-text fLT-Regular-sB `}>
           <StateSelect isProps={{ value: userProfileLocation?.state?.name, location: getAllCountryFromResponse?.state }} ref={refe} />
         </div>
       </Form.Group>
       {/* Area [Section] */}
       <Form.Group as={Col} md={6} className="'mb-3 mb-md-0">
         {/* Area [Option]  */}
         <div className={`uLT-bd-f-platinum-sA uLT-f-radius-sB cLT-main-text fLT-Regular-sB `}>
           <AreaSelect isProps={{ value: userProfileLocation?.area?.name, location: getAllCountryFromResponse?.area }} ref={refe} />
         </div>
       </Form.Group>
