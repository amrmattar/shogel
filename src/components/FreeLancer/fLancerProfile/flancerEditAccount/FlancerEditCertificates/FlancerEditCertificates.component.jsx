...
import { getCertificate, deleteCertificate } from '../../../../../core/redux/reducers/CertificateSkillsReducer.core'
...
const FlancerEditCertificatesComponent = () => {
    ...
    const handleClick = (e) => {
        ...
        if (certificateRef.current?.value === '') {
            ...
            dispatch(getCertificate({ skill: certificateVal, id: Date.now().toString(), level_id: certificateData?.fCertificateRate, type: 'skill' }))
            setCertificateVal('')
            setType(true)
        }
    }
        ...
        <>
            ...
            <Row className="mb-3 d-flex align-items-center gap-4  m-0">
                ...
                <Form.Group as={Col} md={5} className='px-0' >
                    <Form.Control className={`${requireds ? 'uLT-bd-f-danger-sA' : 'uLT-bd-f-platinum-sA'} uLT-f-radius-sB cLT-main-text fLT-Regular-sB`} type='text' placeholder="اسم المهارة"
                        onBlur={() => { setRequired(false) }}
                        ref={certificateRef}
                        name="fCertificate"
                        onChange={(e) => certificateValue(e)}
                        value={certificateVal}
                        required={certificateData.fCertificate.length === 0 ? 'required' : ''}
                    />
                </Form.Group>
                        ...
                        return (
                            <div className="d-flex align-items-center gap-3 uLT-f-radius " key={id} >
                                <div className="d-flex align-items-center gap-3 uLT-f-radius uLT-bd-f-platinum-sA cLT-secondary-bg p-2">
                                    ...
                                    <i className='iLT-Close-button-white iLT-sB uLT-img-contain uLT-click' onClick={() => { dispatch(deleteCertificate(id)) }}></i>
                                </div>
            ...
            {/* Add Certificates Skills [Button] */}
                <div className="d-flex justify-content-end align-items-center px-0">
                    <div className="uLT-f-radius-sB uLT-bd-f-secondary-sA uLT-click d-flex align-items-center justify-content-center gap-2 px-3 py-1">
                        ...
                        <ButtonShare onClick={(e) => handleClick(e)} innerText="اضف مهارة" textClasses="fLT-Regular-sD cLT-secondary-text px-0 py-0" btnClasses="px-0" />
                    </div>
                </div>
            </Row>
        </>
