...
const FlancerOfferPriceForm = ({ data }) => {
    ...
    return (
        <>
            ...
            <form onSubmit={(e) => handleCLickFreelancerForm(e)} className="LT-request-form-grid py-4 mt-4 px-4">
                <label className='fLT-Bold-sA cLT-main-text ' htmlFor="">المشتغل</label>
                <div className="d-flex justify-content-between uLT-bd-f-platinum-sA uLT-f-radius-sB p-4">
                    {/*  FreeLancer [Icon - Circel] */}
                    <FlancerPersonalInformationComponent myData={data} statusIcon={data?.status === 1 ? <div className="uLT-status-online"></div> : <div className="uLT-status-offline"></div>} />
                    {/* List Card Rate */}
                    <div className="d-flex justify-content-center align-items-start ">
                        <div className="ms-3">
                            <CircularStatic num={parseInt(data?.performance ? data?.performance : 0)} />
                        </div>
                        <p className="m-0 card-text cLT-support2-text fLT-Regular-sB">({data?.rate?.count})</p>
                        <p className="m-0 card-text cLT-support2-text fLT-Regular-sB ">{data?.rate?.rate}</p>
                ...
                <div className="LT-details-request position-relative">
                    ...
                    {/* <textarea
                        className=" form-control  p-3 uLT-bd-f-platinum-sA fLT-Regular-sB cLT-main-text" rows="2" placeholder="صف مشروعك هنا"
                        name='description'
