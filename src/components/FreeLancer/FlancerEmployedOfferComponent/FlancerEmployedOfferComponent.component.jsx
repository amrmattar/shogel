...
const FlancerEmployedOfferComponent = ({
  ...
  const routeToFlancerProfile = (e) => {
    ...
    if (
      btnTarget !== "commentButton" &&
      btnTarget !== "commentSend" &&
      btnTarget !== "commentInput" &&
      btnTarget !== "commentHolder"
    ) {
  ...
  const handleSendComment = (e) => {
    ...
    if (commentData.comment !== "") {
      userOfferRequest
        ._POST_MyOfferComment(commentData)
        .then((res) => {
  ...
  const [priceOrTime, setPriceOrTime] = useState({
    price: data?.price,
    time: data?.time,
  });
  ...
  const saveChanged = () => {
    ...
    userOfferRequest
      ._POST_UpdateOfferRequest(data?.id, formData)
      .then((res) => {
  ...
  const handleEditPrice = () => {
    ...
    userOfferRequest
      ._POST_EditPriceRequest(data?.id)
      .then((res) => {
        dispatch(
          getMessages({
            messages: res.data.message,
  ...
  const handleAcceptOffer = () => {
    ...
    userOfferRequest
      ._POST_AcceptOfferRequest(data?.id)
      .then((res) => {
        if (res.data.status === 1) {
          dispatch(
            getMessages({
  ...
  return (
    <>
      <CancelationShared
        cancelRefesh={isComment}
        cancelType={cancelIsType}
        refe={cancelTaskRef}
      ...
      <div
        className="uLT-click~ LT-employed-offer-listCard-grid uLT-bd-f-platinum-sA uLT-f-radius-sB"
        onClick={(e) => routeToFlancerProfile(e)}
        style={{ userSelect: "none" }}
      >
        {/*  FreeLancer [Icon - Circel] */}
        <div
          className="LT-employed-offer-icon "
            ...
            <div className="d-flex align-items-center gap-3 LT-price-time">
              <div className=" d-flex flex-column align-items-center">
                <div className="LT-price d-flex justify-content-center align-items-center gap-2">
                  <i
                    className={` iLT-Advs-ksa-currency uLT-img-contain iLT-sA `}
                  ></i>
                ...
                {checkEdit === true ? (
                  <Form.Control
                    onChange={handlePriceOrTimeChanged}
                    value={priceOrTime?.price}
                    style={{ width: "100px" }}
                    name="price"
                    className="py-0 px-0 uLT-bd-f-platinum-sA"
                    type="number"
                  />
                ) : (
                  <p className="mb-0 cLT-support2-text fLT-Bold-sm-sB ">
                    {" "}
                    <span> {data?.price} </span> ريـــال{" "}
                  </p>
                )}
              </div>
              <div
                className="LT-employed-offer-divied~ uLT-bd-f-platinum-sA"
                style={{ width: "2px", height: "61px" }}
              ></div>
              <div className="d-flex flex-column align-items-center">
                <div className="LT-time d-flex justify-content-center align-items-center gap-2">
                  <i
                    className={` iLT-Advs-calendar uLT-img-contain iLT-sA `}
                  ></i>
                  <p className="mb-0 cLT-support2-text fLT-Regular-sB">
                    {" "}
                    مدة التنفيذ{" "}
                  ...
                  data?.status?.name !== "in proggress" && (
                    <div className="LT-Button-style d-flex justify-content-center align-items-center gap-2 ">
                      <ButtonShare
                        ...
                        innerText="رفض العرض"
                        textClasses="fLT-Regular-sB  px-3 cLT-support2-text"
                        style={{ width: "max-content" }}
                        btnClasses="LT-delete-button cLT-platinum-bg p-2 justify-content-center d-flex align-items-center gap-2 uLT-f-radius-sB"
                      />
                      {data?.change === 1 && (
                        <ButtonShare
                          loading={refreshEditTasks}
                          onClick={handleEditPrice}
                          innerText="طلب تعديـــل"
                          textClasses="fLT-Regular-sB  px-3 cLT-white-text"
                          style={{ width: "max-content" }}
                          btnClasses=" LT-edit-button cLT-secondary-bg p-2 justify-content-center d-flex align-items-center gap-2 uLT-f-radius-sB"
                        />
                      )}
                      <ButtonShare
                        loading={acceptOffer}
                        ...
                        textClasses="fLT-Regular-sB  px-3 cLT-white-text"
                        style={{ width: "max-content" }}
                        btnClasses=" LT-agree-button  cLT-secondary-bg p-2 justify-content-center d-flex align-items-center gap-2 uLT-f-radius-sB"
                      />
                    </div>
                  )}
                        ...
                        {checkEdit === false ? (
                          <div className="w-100">
                            <ButtonShare
                              ...
                              textClasses="fLT-Regular-sB px-3 cLT-white-text"
                              style={{ width: "max-content" }}
                              btnClasses="h-100 cLT-secondary-bg p-2 justify-content-center d-flex align-items-center gap-2 uLT-f-radius-sB"
                            />
                          </div>
                        ) : (
                          <div className="">
                            <ButtonShare
                              loading={checkSaveEdit}
                              onClick={() => saveChanged()}
                              innerText="حفظ التعديــــل"
                              textClasses="fLT-Regular-sB px-3 cLT-white-text"
                              style={{ width: "max-content" }}
                              btnClasses="h-100 cLT-secondary-bg p-2 justify-content-center d-flex align-items-center gap-2 uLT-f-radius-sB"
                            />
        ...
        {(data.user.id == isMyOffer) | isMyTask ? (
          <div className="LT-employed-offer-amenties">
            <div
              className={`${
                data?.comment?.length !== 0
                  ? "amentiesWithLocation pb-3 "
                  : "amentiesWithLocation p-0 w-100"
              }`}
            >
              {/* Amenties Holder */}
            ...
            <div
              id="writeComment"
              className={`${
                data?.comment?.length <= 1 ? "LT-one-comment" : "LT-comments"
              }`}
            >
              {data?.comment?.map((newComment, idx) => {
                return (
                  <div
                    key={idx}
                    className="my-3 uLT-bd-f-platinum-sA uLT-f-radius-sB p-3"
                  >
                    <div
                      className="LT-employed-offer-icon mb-3"
                      style={{ width: "42px", height: "42px" }}
                    >
                      <FlancerPersonalInformationComponent
            ...
            <div
              className={`${
                data?.comment?.length !== 0 ? "my-2 pt-3" : "my-0 pt-0"
              } uLT-f-radius-sB`}
            >
              <Collapse in={open}>
                <div
                  id="example-collapse-text uLT-f-radius-sB"
                  style={{ border: "1px solid #1EAAAD", borderRadius: "10px" }}
                >
                  <div className="d-flex uLT-f-radius-sB ">
                    <Form.Control
                      ...
                      name="comment"
                      className="uLT-f-radius-sB border-0 py-3"
                      type="text"
                    ...
                    >
                      <button
                        ...
                        className=" w-100 fLT-Regular-sB cLT-secondary-text  p-3"
