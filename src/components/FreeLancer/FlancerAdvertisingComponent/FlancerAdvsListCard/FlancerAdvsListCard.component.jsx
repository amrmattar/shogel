                ...
                {!data?.document[0]?.file.match('.mp4') ?
                    data?.document[0]?.file ?
                        <picture className='LT-advsListCard-img'>
                            <img src={data?.document[0]?.file} alt="" width={'100%'} style={{ height: '177px' }} />
                        </picture>
                        :
                        <picture className='LT-advsListCard-img' style={{ width: '216px', height: '177px' }}>
                ...
                <div className='LT-advsListCard-details'>
                    <div className='info-child'>
                        <div className='d-flex justify-content-between w-100 align-items-start'>
                            {/* User Info [Holder] */}
                            <FlancerPersonalInformationComponent myData={data?.user} statusIcon={data?.user?.available === 1 ? <div className="uLT-status-online"></div> : <div className="uLT-status-offline"></div>} />
                    ...
                    <div className='LT-advsListCard-description'>
                        <FlancerDescriptionTitleComponent fontSize={'titleSize'} descriptionTitle={data?.name} />
                        {roll &&
                            <FlancerDescriptionBodyComponent fontSize={'decriptionSize'} descriptionBody={data?.description} />
                        }
                    </div>
                    {/* Card Amenties */}
                    <div className="LT-advsListCard-amenties">
                        {roll &&
                            <AmentiesShared iconWithLocation={'priceIconWithLocation'} time={data?.created_at_value} currency={data?.currency?.name} price={data?.price} locationName={`${data?.user?.country?.name}, ${data?.user?.city?.name}`} />
                        }
                    </div>
                </div>
            </div>
