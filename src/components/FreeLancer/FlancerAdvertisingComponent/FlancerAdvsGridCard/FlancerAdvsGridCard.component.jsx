...
const FlancerAdvsGridCards = ({ data }) => {
    ...
    return (
        <>
            {/* Advertising Cards */}
            <div className="uLT-f-radius-sB p-3 h-100 cLT-white-bg " style={{ width: ' 100%'}}>
                {/* Advs Image [Section] */}
                {!data?.document[0]?.file.match('.mp4') ?
                    data?.document[0]?.file ?
                        <picture className='LT-advsListCard-img '>
                            <img src={data?.document[0]?.file} alt="" width={'100%'} style={{ height: '177px', borderRadius: '10px' , aspectRatio:'1/1' }} />
                        </picture>
                        :
                        <picture className='LT-advsListCard-img ' style={{ width: '216px', height: '177px', aspectRatio:'1/1' }}>
                ...
                <div className="d-flex flex-column gap-3 justify-content-between " style={{ minHeight:'129px'}}>
                    {/* Card title */}
                    <div className="">
                        <p className="m-0 pb-2  fLT-Bold-sm-sA cLT-support2-text">{data?.name}</p>
                        <p className={`m-0 ${location.pathname === '/' ? 'w-100' : '' } fLT-Regular-sA cLT-smoke-text LT-advsCategory-ellipsis`}  dangerouslySetInnerHTML={{__html: data?.description !== 'undefined' ? data?.description : 'لايوجد وصف للإعلان'}}>
                    </p>
                    </div>
                    {/* Card Amenties */}
                    <div className="d-flex align-items-center px-0">
                        <div className="d-flex justify-content-center aling-items-center ps-4">
                            ...
                            <p className="mb-0 cLT-secondary-text fLT-Bold-sm-sA"> <span> {data?.price} </span> {data?.currency?.name}</p>
                        </div>
                        <div className="d-flex justify-content-center aling-items-center">
                            <i className={`iLT-Advs-calendar uLT-img-contain iLT-sA ms-2`}></i>
                            <p className="mb-0 cLT-secondary-text fLT-Bold-sm-sA">قبل {data?.created_at_value}</p>
                        </div>
                    </div>
                </div>
            </div>
