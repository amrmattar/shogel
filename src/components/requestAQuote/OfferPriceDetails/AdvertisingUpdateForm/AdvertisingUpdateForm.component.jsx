...
const AdvertisingUpdateFormComponent = ({ advsId }) => {
    const [getAllUserUpdate, messages] = useSelector((state) => [state.profileUpdate, state.messages])
    ...
    const advsupdateform = useMemo(() => {
        return advertisingLists._GET_MyAdvsById(advsId).then(res => {
    ...
    useEffect(() => {
        const timeout = setTimeout(() => { if (!loadAdvsData) { return advsupdateform } }, 1200);
    ...
    const userLoginData = useMemo(() => {
        return userProfile._GET_ProfileByToken(localStorage.getItem('userTK')).then(res => {
            setUserCategory(res.data.data?.category)
        })
    }, [])
    useEffect(() => {
        const timeout = setTimeout(() => {
            if (!userCategory) { return userLoginData }
        }, 1200);
        return () => clearTimeout(timeout)
    ...
    const handleCLick = async (e) => {
        e.preventDefault()
        setLoading(true)
        dispatch(getMessages({ messages: 'جارى إرسال التحديث', messageType: 'warning', messageClick: true }))
        // @Param Block Of Files Get All And Initial Before Post */
        file.images?.map((image, idx) => { media.append(`images[${idx}]`, image) });
        file.videos?.map((video, idx) => { media.append(`videos[${idx}]`, video) });
        media.set('name', formData.name ? formData.name : loadAdvsData.name)
        media.set('description', content?.value ? content?.value : loadAdvsData?.description)
        media.set('price', formData.price ? formData.price : loadAdvsData.price)
        loadAdvsData?.category?.forEach((newCate, idx) => { media.append(`category[${idx}]`, newCate.id) });
        // @Param POST Method To API */    
        advertisingLists._POST_UpdateAdvertisingForm(advsId, media).then(res => {
    ...
    const handleDelete = (e, fileNewName, i) => {
        if (e.nativeEvent.path[4].id === 'responseUploadData' || e.nativeEvent.path[5].id === 'responseUploadData') {
            deleteBasicData._Delete_Data(i).then(res => { return res })
        }
        const newfileImage = file.images.filter(element => element.name !== fileNewName);
        const newfileVideo = file.videos.filter(element => element.name !== fileNewName);
            ...
            <Form onSubmit={handleCLick} className="LT-advs-form-grid py-4 mt-4 px-4">
                ...
                <Row className="mb-3 gap-3 flex-column m-0">
                    <div className="d-flex gap-3 ps-0 ps-md-3 pe-0 mx-0 flex-column flex-md-row">
                        <Form.Group as={Col} sm={12} md={6} controlId="formGridPassword" className='position-relative px-0 d-grid gap-2' >
                            <Form.Label className="form-label fLT-Bold-sA cLT-main-text m-0"> عنوان الطلب</Form.Label>
                            <Form.Control
                                defaultValue={loadAdvsData?.name !== null ? loadAdvsData?.name : formData?.name}
                                name="name" onChange={handleChange}
                                className='uLT-bd-f-platinum-sA uLT-f-radius-sB' type='text'
                                placeholder="علي سبيل المثال , ببناء موقع علي شبكة الانترنت" />
                            <div className="">
                                {errMessage?.name && <p className='position-absolute mb-0 fLT-Regular-sA cLT-danger-text pt-2¬ px-2'>{errMessage?.name}</p>}
                            </div>
                        </Form.Group>
                        <Form.Group as={Col} sm={12} md={6} controlId="formGridPassword" className='px-0 d-grid gap-2' >
                            <Form.Label className="form-label fLT-Bold-sA cLT-main-text m-0"> السعر</Form.Label>
                            <Form.Control name="price" onChange={handleChange}
                                defaultValue={loadAdvsData?.price !== null ? loadAdvsData?.price : formData?.price}
                                className='uLT-bd-f-platinum-sA uLT-f-radius-sB' type='number'
                                min={1}
                                placeholder="0 ريال" />
                            <div className="">
                                {formData.price < 0 ? <p className='position-absolute mb-0 fLT-Regular-sA cLT-danger-text pt-2¬ px-2'>value must not be 0 or negative number</p> : false}
                                {errMessage?.price <= 0 && <p className='position-absolute mb-0 fLT-Regular-sA cLT-danger-text pt-2¬ px-2'>{errMessage?.price}</p>}
                            </div>
                        </Form.Group>
                    </div>
                </Row>
                    ...
                    <TextEditorShared setDescription={setContent} data={loadAdvsData?.description} setMaxLength={setGetDescriptionLength} characterLength={advsMaxCharacters} />
                ...
                <div className="LT-upload-advs">
                    <Upload inputRef={inputRef} isDrop={fileHandler} targetClick={filePicker}
                        isHaveData={loadAdvsData?.document}
                        fileArr={filenames} handleDelete={handleDelete}
                ...
                <div className="d-grid gap-3 pb-4 h-100">
                    {/* [Title] */}
                    <p className="m-0 fLT-Bold-sA cLT-main-text">ما هي المهارات المطلوبة؟</p>
                    <FlancerEditTagsComponent tags={loadAdvsData?.category} />
                    <div className='d-flex align-items-center gap-3 flex-wrap' >
                        {adsCategory?.map((tag, ix) => {
                            return <div key={ix} id={tag?.id} className="css-4vit6s-multiValue d-flex" style={{ width: 'max-content' }}>
                                <div className="css-12jo7m5">
                                    {tag?.name}
                                </div>
                                <div className="css-o833bd" onClick={(e) => handleTest(tag?.id)} >
                                    <svg height="14" width="14" viewBox="0 0 20 20" aria-hidden="true" focusable="false" className="LT-close-hover-red css-tj5bde-Svg">
                                        <path className='' d="M14.348 14.849c-0.469 0.469-1.229 0.469-1.697 0l-2.651-3.030-2.651 3.029c-0.469 0.469-1.229 0.469-1.697 0-0.469-0.469-0.469-1.229 0-1.697l2.758-3.15-2.759-3.152c-0.469-0.469-0.469-1.228 0-1.697s1.228-0.469 1.697 0l2.652 3.031 2.651-3.031c0.469-0.469 1.228-0.469 1.697 0s0.469 1.229 0 1.697l-2.758 3.152 2.758 3.15c0.469 0.469 0.469 1.229 0 1.698z"></path>
                                    </svg>
                                </div>
                            </div>
                        })}
                        {errMessage?.category && <p className=' position-absolute mb-0 fLT-Regular-sA cLT-danger-text  px-2' style={{ bottom: '170px' }}>{errMessage?.category}</p>}
                    </div>
                </div>
                <div className="d-flex align-items-center justify-content-between">
                    ...
                    <div className="d-flex justify-content-end  align-items-left">
                        <div className="shadow uLT-f-radius-sB">
                            <ButtonShare ID={'send'} loading={isLoading} btnClasses="cLT-secondary-bg py-2 px-4 uLT-f-radius-sB" textClasses="px-4 cLT-white-text fLT-Regular-sC" innerText=" إرسال" />
                        </div>
                    </div>
                </div>
                {/* [Request Button
