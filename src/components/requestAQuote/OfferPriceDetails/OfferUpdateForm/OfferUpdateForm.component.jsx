...
const OfferUpdateFormComponent = ({ taskId }) => {
    ...
    const advsupdateform = useMemo(() => {
        return userOfferRequest._GET_MyTaskById(taskId).then(res => {
            setTaskData(res.data.data)
        }).catch(err => { return err.response })
    }, [taskId, refreshDelete])
    useEffect(() => {
        const timeout = setTimeout(() => { if (!loadTaskData) { return advsupdateform } }, 800);
    ...
    const getCoreData = useMemo(() => {
        ...
        return RegisterServices.GET_RegisterData(modal, selectedCountry, selectedCity).then(res => {
    ...
    function CountrySelect(isProps = {}, ref) {
        return <Select placeholder="البلد"
            ...
            defaultInputValue={loadTaskData?.country?.name ? loadTaskData?.country?.name : isProps.isProps.value}
    ...
    function CitySelect(isProps = {}, ref) {
        return <Select placeholder="المنطقة"
            ref={refe}
            options={isProps.isProps.location}
            defaultInputValue={selectedCityName ? selectedCityName : loadTaskData?.city?.name}
    ...
    function StateSelect(isProps = {}, ref) {
        return <Select placeholder="المدينة"
            ref={refe}
            options={isProps.isProps.location}
            defaultInputValue={selectedStateName ? selectedStateName : loadTaskData?.state?.name}
            onChange={fetchState}
    ...
    const handleTypeOfWork = useCallback(
        () => {
            switch (loadTaskData?.type_work) {
                case "online":
                    return onLineWorkType.current.setAttribute("checked", true) && offLineWorkType.current.setAttribute("checked", false)
                case "offline":
                    return offLineWorkType.current.click() && onLineWorkType.current.setAttribute("checked", false)
                default:
                    return onLineWorkType.current.setAttribute("checked", true) && offLineWorkType.current.setAttribute("checked", true)
            }
        },
    ...
    const [formData, setFormData] = useState({
        ...
        type_work: loadTaskData?.type_work, address: '',
    })
    ...
    const handleCLick = async (e) => {
        ...
        newfile.videos?.map((video, idx) => { return offerPrice.append(`videos[${idx}]`, video) });
        ...
        offerPrice.set('description', content?.value ? content?.value : loadTaskData.description)
        offerPrice.set('time', formData.time ? formData.time : loadTaskData.time)
        offerPrice.set('type_work', formData.type_work ? formData.type_work : loadTaskData?.type_work)
        offerPrice.set('country_id', selectedCountry ? selectedCountry : loadTaskData?.country.id)
        offerPrice.set('city_id', selectedCity ? selectedCity : loadTaskData?.city.id)
        //  If Case ==> The Work Type Offline Get Extra Data Like [//TODO ==> State - Address //]
        if (formData.type_work === 'offline') {
            offerPrice.set('address', formData.address ? formData.address : loadTaskData.address)
            offerPrice.set('state_id', selectedState ? selectedState : loadTaskData?.state?.id)
        }
        ...
        if (getAllUserUpdate?.category?.length !== 0) {
            ...
            loadTaskData.category.forEach((cate, idx) => { offerPrice.append(`category[${idx}]`, cate.id) })
        }
        // TODO ==>  Send Our Collect Data To This [API] 
        userOfferPrice._UPDATE_TaskByID(loadTaskData?.id, offerPrice).then(res => {
            setTaskCheck(false)
            dispatch(getMessages({ messages: res.data.message, messageType: 'success', messageClick: true }))
        ...
        <>
            ...
            <Form onSubmit={(e) => handleCLick(e)} className="LT-request-form-grid py-4 mt-4 px-4 uLT-f-radius-sB">
                ...
                <Row className="mb-3 flex-column m-0">
                    <div className="d-flex gap-3 ps-0 ps-md-3 pe-0 mx-0 flex-column flex-md-row">
                        <Form.Group as={Col} sm={12} md={12} controlId="formGridPassword" className='position-relative px-0 d-grid gap-2' >
                            ...
                            <Form.Control
                                defaultValue={loadTaskData?.name !== null ? loadTaskData?.name : formData?.name}
                ...
                <div className="LT-details-request position-relative">
                    ...
                    <TextEditorShared setDescription={setContent} data={loadTaskData?.description} setMaxLength={setGetDescriptionLength} characterLength={maxCharacters} />
                ...
                <Row className="mb-3 flex-column m-0 pt-3">
                    <div className="d-flex gap-3 ps-0 ps-md-3 pe-0 mx-0 flex-column flex-md-row p-0">
                        <Form.Group as={Col} sm={12} md={6} controlId="formGridTime" className='position-relative px-0 d-grid gap-2' >
                            ...
                            <Form.Control
                                defaultValue={loadTaskData?.time ? loadTaskData?.time : formData?.time}
                            ...
                            <div className="d-flex justify-content-around align-items-center uLT-bd-f-platinum-sA fLT-Regular-sB uLT-f-radius-sB cLT-main-text"
                                ...
                                <div className="fLT-Regular-sC  cLT-main-text text-center ">
                                    <label id="showCompo" className='uLT-click' >
                                        <input type="radio" name="type_work" value="online" datatype='anuone' alt='true' ref={onLineWorkType} />
                                        <span> عن بعد</span>
                                    </label>
                                </div>
                                ...
                                <div className="fLT-Regular-sC  text-center ">
                                    <label id="offlineWork" className='uLT-click' >
                                        <input type="radio" name="type_work" value="offline" datatype='anuone' ref={offLineWorkType} alt='true' />
                                        <span> بالحضور </span>
                                    </label>
                                </div>
                            </div>
                ...
                {formData.type_work === 'offline' &&
                    ...
                    <Form.Label className='fLT-Regular-sB cLT-support2-text '> المنطقة </Form.Label>
                        {/* State [Option]  */}
                        <div className={` uLT-bd-f-platinum-sA uLT-f-radius-sB cLT-main-text fLT-Regular-sB `}>
                            <StateSelect isProps={{ value: getAllCountryFromResponse?.state[0]?.name, location: getAllCountryFromResponse?.state }} ref={refe} />
                        </div>
                        {/* {!location?.citiesID && messages?.city_id && <p className='mb-0 fLT-Regular-sA cLT-danger-text pt-2 px-2'>{messages?.city_id}</p>} */}
                    </Form.Group>
                        <Form.Group as={Col} md={6} className="mb-3" >
                            <Form.Label className='fLT-Regular-sB cLT-support2-text mb-2'>العنوان بالتفصيل</Form.Label>
                            <Form.Control onChange={handleChange} name="address" className='uLT-f-radius-sB uLT-bd-f-platinum-sA cLT-main-text' type='text' value={formData?.address ? formData?.address : loadTaskData?.address} placeholder="العنوان بالتفصيل" />
                        </Form.Group>
                    </Row>
                }
                {/* Upload Files [Holder] */}
                <div className="LT-upload-request">
                    <Upload inputRef={inputRef} isDrop={fileHandler} targetClick={filePicker} fileArr={filenames} handleDelete={handleDelete} isHaveData={loadTaskData?.document}
                ...
                <div className="d-flex align-items-center justify-content-between">
                    ...
                    <div className="d-flex justify-content-end  align-items-left">
                        <div className="shadow uLT-f-radius-sB">
                            <ButtonShare ID={'send'} loading={TaskCheck} btnClasses="cLT-secondary-bg py-2 px-4 uLT-f-radius-sB" textClasses="px-4 cLT-white-text fLT-Regular-sC" innerText=" إرسال" />
                        </div>
                    </div>
                </div>
            </Form>
