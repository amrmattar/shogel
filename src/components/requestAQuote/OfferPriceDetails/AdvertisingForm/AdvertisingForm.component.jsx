...
const AdvertisingFormComponent = () => {
    ...
    const handleCLick = (e) => {
        ...
        setAdvsCheck(true)
        file.images?.map((image, idx) => { return media.append(`images[${idx}]`, image) });
        file.videos?.map((video, idx) => { return media.append(`videos[${idx}]`, video) });
        media.set('name', formData.name)
        media.set('description', content?.value)
        media.set('price', formData.price)
        getAllUserUpdate.category.forEach((cate, idx) => { media.append(`category[${idx}]`, cate) });
        advertisingLists._POST_AdvertisingOffer(media).then(res => {
            setAdvsCheck(false)
    ...
    return (
        <>
            ...
            <Form onSubmit={(e) => handleCLick(e)} className="LT-advs-form-grid py-4 mt-4 px-4">
                ...
                <Row className="mb-3 gap-3 flex-column m-0">
                    <div className="d-flex gap-3 ps-0 ps-md-3 pe-0 mx-0 flex-column flex-md-row">
                        <Form.Group as={Col} sm={12} md={6} controlId="formGridPassword" className='position-relative px-0 d-grid gap-2' >
                            <Form.Label className="form-label fLT-Bold-sA cLT-main-text m-0"> عنوان الاعلان</Form.Label>
                        ...
                        <Form.Group as={Col} sm={12} md={6} controlId="formGridPassword" className='position-relative px-0 d-grid gap-2' >
                            ...
                            <Form.Control value={formData?.price} name="price" onChange={handleChange}
                                className='uLT-bd-f-platinum-sA uLT-f-radius-sB' type='number'
                                placeholder="0 ريال" />
                            <div >
                                {errMessage?.price && <p className='position-absolute mb-0 fLT-Regular-sA cLT-danger-text  px-2'>{errMessage?.price}</p>}
                            </div>
                        </Form.Group>
                    </div>
                </Row>
                ...
                {/* <div className="LT-details-advs position-relative">
                 ...
                 <div className="LT-details-request position-relative">
                    ...
                    <TextEditorShared setDescription={setContent}  setMaxLength={setGetDescriptionLength} characterLength={advsMaxCharacters} />
                    <div className="text-start w-100 cLT-smoke-text">
                ...
                <div className="d-grid  position-relative">
                    {/* [Title] */}
                    <p className="m-0 fLT-Bold-sA cLT-main-text">اختر  <span className='cLT-support1-text'> المجالات</span> المناسبة لاعلانك</p>
                    <FlancerEditTagsComponent
                        categoryClass={'pb-0'}
                        tags={userCategory}
                    />
                    <div >
                        {errMessage?.category && <p className=' position-absolute mb-0 fLT-Regular-sA cLT-danger-text  px-2'>{errMessage?.category}</p>}
                    </div>
                </div>
