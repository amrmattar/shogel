...
const CheckOTPComponent = ({ nextFn }) => {
    ...
    const resendOtpCode = () => {
        dispatch(getMessages({ messages: 'Resending code', messageType: 'warning', messageClick: true }))
    ...
    const mobileOTP = (e) => {
        ...
        setIsloading(true)
        checkOTPValidation._POST_CheckOTPValid(data)
            .then(res => {
                dispatch(getMessages({ messages: res.data.message, messageType: 'success', messageClick: true }))
        ...
        <div className="d-flex justify-content-center ">
            <div className='LT-checkOTP-holder gap-3 '>
                <div className="LT-otp">
                    <OtpInput hasErrored={true} numInputs={4} value={code} onChange={handleChange} />
                </div>
                ...
                <div className="w-100">
                    <ButtonShare
                        ...
                        loading={isLoading} onClick={mobileOTP} innerText={'التحــقق'} btnClasses={'three cLT-secondary-bg'} textClasses={'py-2 cLT-white-text fLT-Regular-sB'} />
                </div>
            </div>
        </div>
    )
