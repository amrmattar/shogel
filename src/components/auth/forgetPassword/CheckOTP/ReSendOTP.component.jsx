...
const onClickResetBtn = () => {
    dispatch(getMessages({ messages: 'جــارى إرســال طلبك', messageType: 'warning', messageClick: true }))
    if(intervalRef.current) clearInterval(intervalRef.current)
    clearTimer(getDeadLineTime())
    MobileServices._POST_ResendOTP({mobile:mobileNumber}).then(res => { 
  ...
  return (
    ...
    <div className='d-flex align-items-center justify-content-between gap-2 gap-sm-4'>
        <Button disabled={timer == '00:00' ? false : true} className='px-0' onClick={() => onClickResetBtn()} >
                <p className={`uLT-list-style fLT-Bold-sm-sA ${timer == '00:00' ? 'cLT-secondary-text' : 'cLT-gray-text'} `}>اعادة الإرســــال</p>
        </Button>
        <div className="cLT-secondary-text">
            {timer}
        </div>
    </div>
     </Fragment>
