...
const RegisterationComponent = ({ isChanged, getFormData, messages, freelancerTagsSection, companySection, setData }) => {
    ...
    const [formInput, setFormInput] = useState({ username: '', fullname: '', email: '', password: '', nationalID: '', commercialRegister: '' })
    ...
    const getFormDatas = useCallback(
        ...
        [dispatch, formInput, jobName, selectedCountry, location, selectedState, getRoleValue, nationality, gender],
    )
    useEffect(() => {
        getFormDatas()
    }, [getFormDatas])
    ...
    return (
        <>
            <div onChange={(e) => setGetRoleValue(e?.target?.value)} className="d-flex flex-column align-items-center justify-content-center p-0">
                <label className='fLT-Regular-sC cLT-support2-text mb-4'> نوع الحساب</label>
                ...
                <div className="LT-user-Type-holder uLT-f-radius-sB p-0 " >
                    ...
                    <div className="fLT-Regular-sC cLT-support2-text  py-3 col  LT-company">
                        <label id="hideCompo" onChange={(e) => isChanged(e)} className='uLT-click' >
                            <input type="radio" name="one" value={4} datatype='anytwo' alt='compTrue' onClick={() => {
                    ...
                    <div className="fLT-Regular-sC cLT-support2-text  py-3 col  LT-freelancer ">
                        <label id="showCompo"
                            ...
                            onChange={(e) => isChanged(e)} className='uLT-click' >
                            <input type="radio" name="one" value={3} datatype='anuone' alt='true' onClick={() => {
                                setOpenFreelancerSection(true)
                            }
                            } />
                    ...
                    <div className="fLT-Regular-sC cLT-support2-text  py-3 col  LT-client ">
                        <label id="hideCompo" onChange={(e) => isChanged(e)} className='uLT-click' >
                            <input type="radio" name="one" value={2} datatype='any' alt='false' onClick={() => {
                ...
                <div className="d-flex gap-3 ps-0 ps-md-3 pe-0 mx-0 flex-column flex-md-row">
                    ...
                    <Form.Group as={Col} sm={12} md={6} controlId="formGridUserName" className='px-0'>
                        ...
                        <Form.Control value={formInput.username.replace(/[^A-Za-z0-9]/ig, "")} name="username" onChange={(e) => handleChange(e)}
                            className='uLT-bd-f-platinum-sA uLT-f-radius-sB cLT-main-text fLT-Regular-sB' type='text' placeholder="اسم المستخدم"
                            maxLength={15}
                            minLength={3}
                        />
                        {messages?.username && <p className='mb-0 fLT-Regular-sA cLT-danger-text pt-2 px-2'>{messages?.username}</p>}
                    </Form.Group>
                    {/* Full Name */}
                    <Form.Group as={Col} sm={12} md={6} controlId="formGridFullName" className='px-0'>
                        <Form.Label className='fLT-Regular-sB cLT-support2-text mb-3'>الاسم بالكامل <span className='cLT-danger-text'>*</span></Form.Label>
                        <Form.Control value={formInput.fullname} name="fullname" onChange={(e) => handleChange(e)}
                            className='uLT-bd-f-platinum-sA uLT-f-radius-sB cLT-main-text fLT-Regular-sB' type='text' placeholder="الاسم بالكامل" />
                        {messages?.fullname && <p className='mb-0 fLT-Regular-sA cLT-danger-text pt-2 px-2'>{messages?.fullname}</p>}
                    </Form.Group>
                </div>
                <div className="d-flex gap-3 ps-0 ps-md-3 pe-0 mx-0 flex-column flex-md-row">
                    {/* Email Address */}
                    <Form.Group as={Col} sm={12} md={6} controlId="formGridEmail" className='px-0  '>
                        ...
                        <Form.Control value={formInput.email} name="email" required onChange={(e) => handleChange(e)} className='uLT-bd-f-platinum-sA uLT-f-radius-sB cLT-main-text fLT-Regular-sB' type="email" placeholder="البريد الإلكتروني" />
                        {messages?.email && <p className='mb-0 fLT-Regular-sA cLT-danger-text pt-2 px-2'>{messages?.email}</p>}
                    </Form.Group>
                    {/* National ID */}
                    {getRoleValue == 4 ?
                        <Form.Group as={Col} md={6} controlId="formGridPassword" className='position-relative' >
                            ...
                            {messages?.commercial_number && <p className='mb-0 fLT-Regular-sA cLT-danger-text pt-2 px-2'>{messages?.commercial_number}</p>}
                        </Form.Group>
                        :
                        <Form.Group as={Col} sm={12} md={6} controlId="formGridNationalID" className='px-0  '>
                            <Form.Label className='fLT-Regular-sB cLT-support2-text mb-3'>رقم الهوية <span className='cLT-danger-text'>*</span></Form.Label>
                            <Form.Control value={formInput.nationalID.replace(/[^0-9]/ig, "")} required name="nationalID"
                                onWheel={(e) => e.target.blur()}
                                onChange={(e) => handleChange(e)} className='uLT-bd-f-platinum-sA uLT-f-radius-sB cLT-main-text fLT-Regular-sB' type="number" placeholder="رقم الهوية" />
                            {messages?.nationality_number && <p className='mb-0 fLT-Regular-sA cLT-danger-text pt-2 px-2'>{messages?.nationality_number}</p>}
                        </Form.Group>
                    }
            ...
            <Row className=" d-flex align-items-center mb-3">
                ...
                {/* Password Title */}
                <Form.Group as={Col} md={4} controlId="formGridPassword" className='position-relative' >
                    ...
                    <Form.Control value={formInput.password} name="password" onChange={(e) => handleChange(e)} className='uLT-bd-f-platinum-sA uLT-f-radius-sB' type='password' placeholder="0000" />
                    ...
                    {formInput?.password?.length < 8 && messages?.password && <p className='position-absolute text-nowrap mb-0 fLT-Regular-sA cLT-danger-text pt-2 px-2'>{messages?.password}</p>}
                </Form.Group>
                {/* Gender */}
                <Form.Group as={Col} md={4} controlId="formGridGender" className='' >
                    {/* Gender [Label] */}
                    <Form.Label className='fLT-Regular-sB cLT-support2-text mb-3 position-relative'>النوع <span className='cLT-danger-text'>*</span></Form.Label>
                    {/* Gender [Option]  */}
                    <div className={`uLT-bd-f-platinum-sA uLT-f-radius-sB cLT-main-text fLT-Regular-sB`}>
                        <Select placeholder="النوع" options={getFormData?.gender}
                            onChange={fetchGender}
                            getOptionLabel={gender => gender?.name} getOptionValue={gender => gender?.id} />
                    </div>
                    {!gender && messages?.gender_id && <p className='position-absolute text-nowrap mb-0 fLT-Regular-sA cLT-danger-text pt-2 px-2'>{messages.gender_id}</p>}
                </Form.Group>
                {/* Nationality */}
                <Form.Group as={Col} md={4} controlId="formGridNationality" className='' >
                    {/* Nationality [Label] */}
                    ...
                    {!nationality && messages?.nationality_id && <p className='position-absolute text-nowrap mb-0 fLT-Regular-sA cLT-danger-text pt-2 px-2'>{messages.nationality_id}</p>}
                </Form.Group>
            </Row>
            {/* fourth  Row [Location] */}
            <Row className='d-flex align-items-center pt-3'>
                {/* Country [Section] */}
                <label className='fLT-Regular-sB cLT-support2-text mb-2'>موقعك <span className='cLT-danger-text'>*</span></label>
                ...
                <Form.Group as={Col} md={4} className="mb-4">
                    {/* City [Option]  */}
                    <div className={` uLT-bd-f-platinum-sA uLT-f-radius-sB cLT-main-text fLT-Regular-sB `}>
                        <Select placeholder="الحى" options={getAllCountryFromResponse?.state}
                            onChange={fetchState} getOptionLabel={state => state?.name} getOptionValue={state => state?.id}
                        />
                    </div>
                    {!location?.stateID && messages?.state_id && <p className='mb-0 fLT-Regular-sA cLT-danger-text pt-2 px-2'>{messages?.state_id}</p>}
                </Form.Group>
            </Row>
            ...
            <Collapse in={openFreelancerSection}>
                <div id="example-collapse-freelancer uLT-f-radius-sB" >
                    ...
                    <Form.Group as={Col} md={12} controlId="formGridGender" className='mt-3' >
                        ...
                        {!jobName && messages?.job_name_id && <p className='mb-0 fLT-Regular-sA cLT-danger-text pt-2 px-2'>{messages?.job_name_id}</p>}
                    </Form.Group>
