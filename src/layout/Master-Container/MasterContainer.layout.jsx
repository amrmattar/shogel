    ...
    return (
        <>
            <div className="overflow-hidden w-100">
                <Routes>
                    <Route path={`/advertising-price/:id`} element={<FlancerAdvertisingOfferPage />} />
                    <Route path={`/update-offer-price/:id`} element={<ClientOfferPricePage />} />
                    <Route element={<UserValidta />}>
                        <Route path={`account_management/*`} element={<FlancreAccountManagementPage />}>
                            <Route path={`my-edit-account/${userID}`} element={<FreelancerMainEditAccountPage />} />
                            <Route element={<PrivateRoutes />}>
                                <Route path={`my-order/${userID}/page=:num`} element={<FlancerMyOrderPage />} />
                                <Route path={`my-advertising/${userID}/page=:num`} element={<FlancerMyAdvsPage />} />
                                <Route path={`my-notification/${userID}`} element={<FlancerMyNotificationPage />} />
                                <Route path={`my-favorite/${userID}/page=:num`} element={<FlancerMyFavoritePage />} />
                                <Route path={`my-wallet/${userID}`} element={<FlancerMyWalletPage />} />
                                <Route path={`my-setting/${userID}`} element={<FlancerMySettingPage />} />
                                <Route path={"my-logout"} element={<FlancerMyLogoutPage />} />
                            </Route>
                            <Route path={"*"} element={<Navigate to={`my-edit-account/${userID}`} />} />
                        </Route>
                    </Route>
