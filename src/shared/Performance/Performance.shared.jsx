...
const PerformanceShared = ({
  ...
  const Star = ({ selected = false, onClick = (f) => f }) => (
    <div
      aria-required="true"
      className={`${selected ? "cLT-secondary-bg" : "cLT-platinum-bg "} && ${
        !status && "uLT-click"
      } `}
  ...
  const getCertificateRating = useCallback(
    (e) => {
  ...
  const getLanguageRating = useCallback(
    (e) => {
    ...
    <div className="d-flex gap-3 align-items-center">
      {[1, 2, 3, 4, 5].map((n, i) => (
        <Star
          ...
          onClick={!status ? () => setStar(i + 1) : undefined}
        />
      ))}
    </div>
  );
